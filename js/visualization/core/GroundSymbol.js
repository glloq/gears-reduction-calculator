...
(function (root, factory) {
  ...
  function ring(cx, cy, radius, options) {
    ...
    var out = [{ tag: 'circle', attrs: { class: 'ground-boundary' + css,
      cx: fixed(cx), cy: fixed(cy), r: fixed(r) } }];
    for (var i = 0; i < count; i++) {
      var a = 2 * Math.PI * i / count;
      var x = cx + Math.cos(a) * r, y = cy + Math.sin(a) * r;
      // Le trait part du contour et fuit vers l'extérieur : incliné de `lean`
      // par rapport au rayon, il donne le peigne caractéristique du bâti.
      var b = a + lean;
      out.push({ tag: 'line', attrs: { class: 'ground-hatch' + css,
        x1: fixed(x), y1: fixed(y),
        x2: fixed(x + Math.cos(b) * length), y2: fixed(y + Math.sin(b) * length) } });
    }
    return out;
  }
