...
(function (GearApp) {
  ...
  WorkbenchUI.prototype._enhanceObjective = function () {
    ...
    if(output){var need=[output.previousElementSibling,output,output.nextElementSibling,output.nextElementSibling&&output.nextElementSibling.nextElementSibling].filter(Boolean);var box=wrap(need,'objective-need');var derived=document.createElement('output');derived.id='derivedRatio';derived.className='derived-ratio';box.appendChild(derived);}
    if(travel){var linear=wrap([travel.previousElementSibling,travel],'objective-linear');linear.insertAdjacentHTML('beforeend','<label for="linear_speed_min">Vitesse linéaire min (mm/min)</label><input id="linear_speed_min" data-persist type="number"><label for="linear_speed_max">Vitesse linéaire max (mm/min)</label><input id="linear_speed_max" data-persist type="number"><label for="linear_force_min">Force sortie minimum (N)</label><input id="linear_force_min" data-persist type="number"><p class="formula">course/tour = π × diamètre primitif</p>');}
  ...
  WorkbenchUI.prototype.updateContext = function () { var mode=el('objective_mode')?el('objective_mode').value:'ratio';document.querySelectorAll('.objective-fields').forEach(function(group){group.classList.toggle('active',group.classList.contains('objective-'+(mode==='rotationTranslation'?'linear':mode)));});var output=el('rpm_sortie_cible');if(output)output.dispatchEvent(new Event('input'));this.updateSummary(); };
