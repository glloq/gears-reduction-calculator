...
(function (GearApp) {
  ...
  function WorkbenchUI(eventBus) { this.bus = eventBus; this.solutions = []; this.selected = 0; this._rotaryTypes = null; }
  ...
  WorkbenchUI.prototype.updateSummary = function () { var ratio=parseFloat(el('rapport').value)||0,mode=el('objective_mode').value,summary=el('configurationSummary');if(mode==='rotationTranslation'){var bits=['<strong>'+finite(parseFloat(el('linear_travel_per_rev').value),2)+' mm/tr</strong>','<span>'+finite(parseFloat(el('vitesse_entree').value),0)+' rpm entrée</span>'];var min=el('linear_speed_min'),max=el('linear_speed_max'),force=el('linear_force_min');if(min&&min.value)bits.push('<span>Vitesse ≥ '+min.value+' mm/min</span>');if(max&&max.value)bits.push('<span>Vitesse ≤ '+max.value+' mm/min</span>');if(force&&force.value)bits.push('<span>Force ≥ '+force.value+' N</span>');bits.push('<span>Pignon-crémaillère</span>');if(summary)summary.innerHTML=bits.join('');return;}if(mode==='need'){var a=parseFloat(el('vitesse_entree').value),b=parseFloat(el('rpm_sortie_cible').value);if(a>0&&b>0)ratio=a/b;}var types=Array.from(document.querySelectorAll('.type-checkbox:checked')).map(function(c){return c.nextElementSibling.textContent;});if(summary)summary.innerHTML='<strong>'+finite(ratio,2)+':1</strong><span>± '+el('precision').value+' %</span><span>'+types.join(' · ')+'</span><span>≤ '+el('etages').value+' étages · '+(OPTIMIZATION_COPY[el('search_mode').value]||'')+'</span>'; };
