...
test('architecture template is parsed, validated against active types and forwarded',()=>{
  let {SearchParams}=makeForm({type_template:'[["spur"],null]'});let p=SearchParams.fromForm();
  // Comparaison JSON : les tableaux issus du contexte vm sont d'un autre realm.
  assert.equal(JSON.stringify(p.typeTemplate),'[["spur"],null]');
  assert.equal(p.validate().valid,true);
  assert.equal(JSON.stringify(p.toWorkerParams().typeTemplate),'[["spur"],null]');
  // Malformé → ignoré silencieusement
  ({SearchParams}=makeForm({type_template:'{oops'}));p=SearchParams.fromForm();
  assert.equal(p.typeTemplate,null);
  // Type imposé non activé (worm absent de typesActifs) → validation bloquante
  ({SearchParams}=makeForm({type_template:'[["worm"]]'}));p=SearchParams.fromForm();
  assert.equal(p.validate().valid,false);
  assert.equal(p.validate().field,'type_template');
